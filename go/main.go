...
func initialize(c echo.Context) error {
	...
	query := "USE `isucon`; CREATE INDEX `idx_user_id_present_history` ON `user_present_all_received_history` (`user_id`);"
	if _, err := dbx.Exec(query); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
	query = "USE `isucon`; CREATE INDEX `idx_present_all_id` ON `user_present_all_received_history` (`present_all_id`);"
	if _, err := dbx.Exec(query); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
